...
def parse_args():
    ...
    p.add_argument("--ext", default=".c,.cpp,.cc,.cxx,.h,.hpp,.hh", help="Comma-separated source file extensions (default: .c,.cpp,.cc,.cxx,.h,.hpp,.hh)")
    return p.parse_args()

# Import the c_cpp parser from ingestion.py
from ingestion import parse_c_cpp

def extract_elements(path):
    """Extracts top-level functions, methods, classes, and structs using ingestion.py parser."""
    elements = []
    try:
        with open(path, 'r', encoding='utf-8') as f:
            content = f.read()
        parsed_data = parse_c_cpp(content, path)
        for item in parsed_data:
            # We are interested in functions, methods, classes, and structs defined in the target file
            if item['type'] in ['function', 'method', 'class', 'struct']:
...
def find_mentions(root, exts, target_basename, elements, target_path):
    ...
    for dirpath, dirnames, files in os.walk(root):
        ...
        dirnames[:] = [d for d in dirnames if not any(p.search(d) for p in exclude_patterns)]
        # Skip processing files within excluded folders
        if any(p.search(part) for part in Path(dirpath).parts for p in exclude_patterns):
            continue
        ...
        for fname in files:
            ...
            try:
                ...
                if re.search(r'\b' + re.escape(target_basename) + r'\b', text):
                 ...
                 # Check mentions for each extracted element (function, method, class, struct)
                for element in elements:
                    # Use word boundary to avoid partial matches
                    # For methods, search for the full name (e.g., MyClass::myMethod or MyClass.myMethod)
                    # For classes/structs, search for the name
                    search_name = element['name']
                    # Handle C++ names with :: or . for methods/nested types
                    # Note: The regex should search within the 'text' variable.
                    if re.search(r'\b' + re.escape(search_name) + r'\b', text) or \
                       (element['type'] == 'method' and re.search(r'\b' + re.escape(search_name.replace('.', '::')) + r'\b', text)):
...
if __name__ == '__main__':
    ...
    # Use the new function to get elements (functions, methods, classes, structs)
    ...
    nodes['Target'] = {'id': 'Target', 'label': Path(target).name, 'dir': target_dir, 'is_file': True}
    for element in elements:
        # Use a consistent prefix for element nodes
        elem_id = f'Elem_{element["name"]}'
        # Indicate type in label, e.g., MyClass (classDefClass) or myFunction() (function) or myMethod() (method) or MyStruct (struct)
        # Change label for 'class' type to '(classDefClass)'
        if element['type'] == 'class':
            label_suffix = " (classDefClass)"
        elif element['type'] == 'struct':
            label_suffix = " (struct)"
        else:
            label_suffix = "()" # Functions and methods
        nodes[elem_id] = {'id': elem_id, 'label': f'{element["name"]}{label_suffix}', 'dir': target_dir, 'is_file': True}
    ...
    edges = []
    processed_file_to_target_links = set() # Track file->Target links to avoid duplicates if element link exists
    ...
    for frm, to in raw_edges:
        ...
        if is_source_file:
            # Case 1: File mentions a specific element (function, method, class, struct)
            ...
            # Case 2: File mentions the Target file itself (e.g., include)
            elif to == 'Target':
    ...
    print('  classDef file fill:#e3f2fd,stroke:#1e88e5,stroke-width:2px,padding:10px;')      # Light blue for files
    print('  classDef folder fill:#fff3e0,stroke:#fb8c00,stroke-width:2px,stroke-dasharray:5 5,padding:10px;') # Light orange for folders
    print('  classDef function fill:#e8f5e9,stroke:#4caf50,stroke-width:2px,padding:10px;') # Greenish for functions
    print('  classDef method fill:#e1bee7,stroke:#ab47bc,stroke-width:2px,padding:10px;')   # Purplish for methods
    # Renamed 'class' classDef to 'classDefClass' to avoid syntax issues
    print('  classDef classDefClass fill:#fce4ec,stroke:#ec407a,stroke-width:2px,padding:10px;')    # Pinkish for classes
    print('  classDef struct fill:#fff9c4,stroke:#fbc02d,stroke-width:2px,padding:10px;')    # Light yellow for structs
    print()
    ...
    for grp, group_nodes in groups.items():
        ...
        label = grp if grp != '.' else 'root' # Use 'root' as label for the root directory
    ...
    for (frm, to), count in edge_counts.items():
        ...
        if not from_node:
            ...
            print(f"Warning: Source node {frm} not found for edge ({frm}, {to}), skipping edge output.")
            continue
    ...
    for nid, data in nodes.items():
        ...
        if nid.startswith('Elem_'):
            ...
            # This assumes element names are unique within the target file's top-level scope for functions/classes/structs
            # Methods are named as Class.method or Class::method, which helps in uniqueness if the class name is unique
            elem_data = next((e for e in elements if e['name'] == original_name), None)
            if elem_data:
                 # Assign 'function', 'method', 'classDefClass', or 'struct' style based on the parsed data type
                 # Use 'classDefClass' for the 'class' type
                 style = 'classDefClass' if elem_data['type'] == 'class' else elem_data['type']
